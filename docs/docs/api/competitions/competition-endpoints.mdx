...
| Field   | Type                                                 | Required | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------- | ---------------------------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| metrics | [Metric](/api/global-type-definitions#enum-metric)[] | `false`  | (chain multiple with ?metrics=A&metrics=B&metrics=C) Overrides the competition's own metrics, to preview its progress in other metrics. (Ex: If it's originally an `agility` competition, you can use this param to preview what the competition would have looked like if it were a `fishing` competition instead.) When set, each participation's `deltas` will contain one entry per given metric (plus a `"total"` entry if more than one metric is given), and participants will be ranked by the combined gains of all the given metrics. |

<br />

:::caution
The `progress` and `levels` fields on each participation are **deprecated** in favor of `deltas`, and will be removed in a future release. They currently mirror the entry in `deltas` that the competition is ranked by (the `"total"` entry when there's more than one metric, or the single metric's entry otherwise).
:::
