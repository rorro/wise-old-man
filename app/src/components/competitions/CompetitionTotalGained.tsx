...
export function CompetitionTotalGained() {
  ...
  const sum = competition.participations.reduce(
    (acc, p) => acc + (p.deltas.find((d) => d.metric === selectedMetric)?.values.gained ?? 0),
    0,
  );
  ...
  return (
    <div
      className={cn(
        ...
        selectedMetric === "total" && "bg-gray-800",
      )}
    >
      <ImageWithFallback
        alt={selectedMetric}
        ...
        src={`/img/backgrounds/${selectedMetric === "total" ? "overall" : selectedMetric}.png`}
      />
      ...
      <div className="z-1 relative flex flex-col gap-y-0.5">
        <span className="line-clamp-1 text-xs text-gray-100">
          {selectedMetric === "total" ? "Total gained" : MetricProps[selectedMetric].name}
        </span>
